/*****************************************************************************
 * Queries over the tours of Paris, discrete and continuous version.
 *
 * A tour is a sequence of visits to points of interest. The discrete version
 * reads the view TourPoI, one row per visit, joining table Tour with the
 * description of the point of interest. The continuous version reads table
 * TempTour, one row per tour, holding the succession of visits as a single
 * tjsonb value and the itinerary as a tgeompoint.
 *
 * The tours are semantic trajectories in which every step carries a document
 * rather than a measure, so the queries are patterns over the succession of
 * documents. They exercise the elements a pattern language needs: the
 * sequence itself, an optional element, an anchor on the first or the last
 * step, a variable bound across two steps, and a condition mixing the
 * document with time or with space.
 *
 * In the continuous version a step is obtained by restricting the tjsonb to
 * the time when a condition on the document holds, and the before of the
 * pattern is the temporal before operator <<#. Note that comparing two
 * temporal values with < would return the same answers on this data, because
 * temporal values are ordered by their bounding box and hence by their period
 * first, and the steps of a tour are disjoint. That coincidence does not
 * survive steps that overlap: for those, < falls back on the upper bound and
 * then on the values, which has nothing to do with the order of time. The
 * pattern means before, so it is written <<#.
 *****************************************************************************/
...
/* The optional element of the pattern, [Category = 'Restaurant']*, is a left
 * outer join in both versions: a pattern element that may be absent is not a
 * restriction but an enrichment of the match. What differs is the step: the
 * discrete version reads a row of the view, the continuous version restricts
 * the tour to the time when the document satisfies the condition. Both return
 * the same two tours, one of which has no restaurant between the hotel and
 * the movie. */
...
/* The plainest form of the pattern: two conditions on the document and a
 * before between them. The discrete version compares the step numbers, which
 * is available only because the visits are numbered in the data; the
 * continuous version compares the times, which is available always. The two
 * agree here because the numbering follows the time. */
...
/* The anchor ^ of the pattern selects the first step. The discrete version
 * writes it as StepNo = 1 and the continuous one as a condition inside the
 * JSON path, both relying on the step number stored in the document. Written
 * on the time instead, the anchor is the start of the tour, which needs no
...
/* The anchor $ selects the last step. In the discrete version it is a
 * correlated subquery computing the maximum step number of the tour. In the
 * continuous version the last step of a temporal value is reached directly,
...
/* Same shape as the first query with the optional element in the middle. */
WITH FrenchRestaurant(TourId, Tour) AS (
  SELECT TourId, atTime(Tour, unnest(spans(whenTrue(
...
/* The pattern binds a variable, @x, to the price of the first place and
 * requires the last place to have the same one. A variable shared by two
 * pattern elements is an equality between two steps of the same match, which
 * both versions express as a join condition, the discrete one on rows and the
 * continuous one on the two restricted temporal values. */
...
/* The condition mixes the document with the time of the step. The temporal
 * value carries its own time, so the hour is read from the step itself
 * instead of from a separate column. */
...
/* The part of the day is a function of the instant of the step, not a value
 * in the document. In the continuous version segments(Tour) turns the tour
 * into its steps, each of which keeps its period, and startTimestamp gives
 * the instant from which the part of the day is computed. The two versions
 * return the same tour. */
...
/* The condition is on the itinerary and not on the document, so it is
 * evaluated on the tgeompoint: valueAtTimestamp gives the place of the tour
 * at the start of the step, which is the point the discrete version reads
 * from the view. The distances differ in the tenth decimal because the
 * continuous version recovers the point from the temporal value while the
 * discrete one reads the geometry of the point of interest. */
