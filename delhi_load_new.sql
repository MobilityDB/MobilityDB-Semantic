/*****************************************************************************
 * Pipeline that builds the Delhi semantic trajectories, in two segmentation
 * models so that the discrete and the continuous queries can be compared on
 * equal terms.
 *
 * MODEL A - segmentation only (table TripVisits, added by this file)
 *   A row is one visit of a trip to one cell: the cell and the time span of the
 *   visit. The measured values stay in TripPoints, where they are observed, so a
 *   query reads them at the instants they were measured rather than an average
 *   formed when the table was built.
 *
 *   A visit begins at the first observed point inside the cell and ends at the
 *   last, where a visit of Model C begins and ends at the crossing of the
 *   boundary. The cells of a trip are therefore those holding an observed
 *   point, its visits are contained in the visits of Model C, and a value is
 *   the mean of the points rather than the mean over the time. The three
 *   differences are measured in delhi_model_comparison.sql, which also gives
 *   what the model costs.
 *
 * MODEL B - segmentation with per-visit averages (table TripCells, unchanged)
 *   A row is one visit together with the average of the values observed during
 *   it. This is what a semantic-trajectory pipeline usually stores. Averaging
 *   discards the variation inside a visit and makes the number of rows of a trip
 *   depend on the grid, so queries over this model answer a different question
 *   from the continuous ones, and the difference measures the cost of the
 *   summarisation.
 *
 * MEASURED on this database for the query "trips where the Pm25 increases
 * throughout the entire trip": the continuous approach returns 708 trips, the
 * discrete one over Model A returns the same 708 with an empty symmetric
 * difference, and the discrete one over Model B returns 1102, of which 57 are
 * also in the continuous answer. The 651 trips Model B misses all visit a single
 * cell, so they have no consecutive pair for the COUNT(*) >= 2 guard; 21726 of
 * the 27118 trips visit a single cell. The 1045 it adds have rising per-cell
 * averages while the value falls somewhere inside a cell.
 *
 * The Model A equality is structural, not a property of this dataset: Pm25 is
 * stored with linear interpolation, so the value between two observations moves
 * monotonically between them, and "increasing at every instant" is the same
 * statement as "increasing between consecutive observations". The same holds for
 * any pointwise condition. It does not hold for conditions on durations, where
 * the discrete approach measures a visit from its first to its last observation
 * while the continuous one measures the exact extent of the condition.
 *****************************************************************************/
...
-------------------------------------------------------------------------------
  ...
  RAISE NOTICE 'Creating the table TripVisits';

  -- Model A: the same segmentation as TripCells, with no value aggregated. The
  -- values are read from TripPoints when a query needs them, which keeps the
  -- discrete approach on the same values as the continuous one.

  DROP TABLE IF EXISTS TripVisits;
  CREATE TABLE TripVisits(TripId integer, StepNo integer, CellId integer,
    StartTime timestamptz, EndTime timestamptz,
    PRIMARY KEY (TripId, CellId, StartTime));
  INSERT INTO TripVisits
  WITH PointCells(TripId, T, CellId, PrevCell) AS (
    SELECT p.TripId, p.T, g.CellId,
      LAG(g.CellId) OVER (PARTITION BY p.TripId ORDER BY p.T)
    FROM TripPoints p, Grid g
    WHERE g.Box @> stbox(p.Geom) ),
  StepStarts(TripId, T, CellId, NewStep) AS (
    SELECT TripId, T, CellId,
      CASE WHEN PrevCell IS NULL OR CellId <> PrevCell THEN 1 ELSE 0 END
    FROM PointCells ),
  Steps(TripId, StepNo, T, CellId) AS (
    SELECT TripId, SUM(NewStep) OVER (PARTITION BY TripId ORDER BY T
      ROWS UNBOUNDED PRECEDING), T, CellId
    FROM StepStarts )
  SELECT TripId, StepNo, MIN(CellId), MIN(T), MAX(T)
  FROM Steps
  GROUP BY TripId, StepNo
  ORDER BY TripId, StepNo;

  CREATE INDEX TripVisits_TripId_Idx ON TripVisits(TripId);

  -- INSERT 0 70881, the same segmentation as TripCells
